...
- 履歴保存: `src/app/storage/performanceHistoryStorage.ts`, `src/app/usePerformanceHistory.ts`
- 固定 UI シナリオ: `src/game/uiScenarioRegistry.json`, `src/game/uiScenarios.ts`, `src/game/uiScenarioBuilders.ts`, `src/game/uiScenarioStorage.ts`
...
公演履歴の `localStorage` 読み書きは `src/app/storage/performanceHistoryStorage.ts` に置く。game 層へ直接戻さない。
...
- `GameState` は phase 別 union として扱い、UI固定シナリオで必要な例外は型に明示する。
