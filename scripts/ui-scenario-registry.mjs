...
export function presetJobsFor(kind, presetName) {
  ...
  if (kind === 'check' && presetName === 'mobile-fullscreen') {
    return uiScenarioRegistry.scenarios
      .filter((scenario) => groups.includes(scenario.group))
      .flatMap((scenario) => uiScenarioRegistry.mobileFullscreenCheckViewports.map(([width, height]) => [scenario.name, width, height]));
  }
