...
export const appCopy = {
  ...
  title: {
    ...
    howToLines: [
      ...
      '1日目ソワレ後に公演の色が決まり、2日目以降の見せ場・余韻・安定・収束の意味が少し変わる。',
    ],
    howToSteps: [
      { title: '兆候を見る', body: '注目役者の状態と出やすい出来事を読む。' },
      { title: '準備する', body: '本番前メモへ備えを反映し、想定外の受け方を決める。' },
      { title: 'キューを出す', body: '見せ場、余韻、安定、収束から一手を選び、結果を次へ渡す。' },
    ],
    rulebookTitle: '判定仕様',
    rulebookLead: '見立ては確率ではなく、現在の状態、準備、キュー、負荷から出す評価レンジ。',
    rulebookItems: [
      { title: 'ランク閾値', body: '判定点70以上で名場面、40以上で場面化、20以上で小さな成功、0以上でほころび、0未満で事故。' },
      ...
      { title: 'ほころび回収', body: '残ったほころびに合うキューを出すと加点やランク軽減が入り、強く回収できると一段伸びる。' },
  ...
  resultPreview: {
    ...
    emptyBody: '結果は確定前に確認できる。見せ場か、余韻か、安定か、収束か。',
    ticket: 'キュー結果票',
    finale: '千秋楽',
    ratingLabel: 'キュー判定',
    ...
    sceneRecord: '場面記録',
    scoringMemo: 'キューの効き方',
    breakdownSummary: '判定内訳を開く',
    breakdownAria: 'キュー判定の内訳',
    breakdownTotal: '合計',
    breakdownEmpty: '表示する判定内訳はありません。',
    frayRecovery: 'ほころび回収',
    ...
    deltaTitle: '公演への反映',
    deltaSub: '次に残る変化',
    deltaLineAria: (label: string, before: string, after: string, toneLabel: string, change: string) => `${label}: ${before}から${after}、${toneLabel}、${change}`,
    deltaLabels: {
      flow: '段取り',
      trust: '一体感',
      load: '負荷',
...
export function prepMeaningMemo(prep: PrepAction) {
  if (prep === 'watch') return '見せ場の仕込みは、役者の熱がこぼれた瞬間を見逃さないための準備。止めるよりも、出てきた動きを舞台上の意味に変える。';
  if (prep === 'makeSpace') return '余韻の仕込みは、沈黙や退場の間を急かさず残すための準備。何も起きていないように見える時間を、余韻として客席へ渡す。';
  if (prep === 'tightenFlow') return '安定の仕込みは、立ち位置やテンポのズレを舞台全体の呼吸へ戻すための準備。熱を削りすぎず、進行の軸を支える。';
  return '収束の仕込みは、崩れを大きくする前に小さく閉じるための準備。場面を伸ばすより、次の回へ渡す道筋を残す。';
}
...
export function scoreMoodMemo(kind: 'scene' | 'flow' | 'trust', value: number) {
  ...
  if (value < 4) return '座組に少し呼吸が残っている。余韻判断や得意筋が、場面の余韻になりやすい。';
