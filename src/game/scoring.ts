...
function prepResponseScoreItem(state: GameState, response: MainResponse, prepQuality: PrepPredictionQuality): ScoreBreakdownItem | undefined {
  ...
  if (prepQuality === 'hit') {
    return scoreItem('prep-response', `${PREP_LABELS[prep]}の主対応が乗った`, 1, `${RESPONSE_LABELS[response]}の上振れを少し広げた。`);
  }
  if (prepQuality === 'partial') {
    return scoreItem('prep-response', `${PREP_LABELS[prep]}の主対応を選んだ`, 0, '見えていた兆候には沿っているため、判断が安定する。');
  }
  return undefined;
}
...
export function responseInsight(state: GameState, response: MainResponse): ResponseInsight {
  ...
  return {
    ...
    resultTier,
    deltaLoad,
    handTypeLabel: RESPONSE_HAND_TYPES[response],
